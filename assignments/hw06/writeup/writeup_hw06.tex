...
\begin{enumerate}[label=(1\alph*), ref=1\alph*]
    \item Two random forest classifiers with 100 trees each.  One with 22 features, the other with 500.  Table in figure 
    ...
    \end{table}
    \item Compute feature importances using mean decrease in impurity (MDI) and the permutation feature importances.
    \begin{figure}[htbp] 
        \centering 
    ...
    \end{figure}
    \item Selecting \(k \in\{10, 20, 30, 40\}\) most important features by the two methods mentioned above 
    on the two classifiers from part a, 
    train RF classifier where each split is from all 500 features.  
    The test and train errors are reported in table \ref{tab:table_part_c}.  
    \begin{table}[htbp]
        \centering
    ...
    \end{table}
    \item For \(k \in\{10, 20, 30, 40\}\), two plots, one of each depth \(d\in\{4,5\}\).  
    ...
    \end{figure}
    Of the 320 classifiers, we can report the configuration with the lowest test error in table: \ref{tab:table_part_d}
    \begin{table}[htbp]
        \centering
    ...
    \end{table}
    \item Of the models, the random forest classifier from part c, which selects the top 30 features 
    according to mean decrease in impurity (MDI) with all 500 features considered at each split, 
    produced the lowest test misclassification error of 0.11667.  
\end{enumerate}
